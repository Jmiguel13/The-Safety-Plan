...
import { myShopLink, type CartItem } from "@/lib/amway"; // <- use storefront helper only
...
export default function BuyButtons({
  ...
  const openBuy = () => {
    // Always open your MyShop storefront (reliable) then show the SKUs modal.
    window.open(myShopLink("quick-buy"), "_blank", "noopener,noreferrer");
  ...
  const copyOne = async (sku: string, qty = 1) => {
    try {
      ...
      // no alert to avoid spam; feels instant
    } catch {}
  };
          ...
          <div className="absolute inset-0 mx-auto flex max-w-lg items-center justify-center p-4">
            <div className="w-full rounded-2xl border border-zinc-800 bg-zinc-950 p-5">
              ...
              <div className="mt-4 flex gap-3">
                <a
                  href={myShopLink("fallback-open-shop")}
                  target="_blank"
                  rel="noopener noreferrer"
