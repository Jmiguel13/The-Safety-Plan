...
import { myShopLink, type CartItem } from "@/lib/amway";
...
export default function BuyButtons({
  ...
  const openBuy = () => {
    // Open MyShop root (reliable) then show the SKUs modal.
    window.open(myShopLink(), "_blank", "noopener,noreferrer");
    setShowFallback(true);
  };

  const copySkus = async () => {
    const text = items.map((i) => `${i.sku} x${i.qty ?? 1}`).join("\n");
    try { await navigator.clipboard.writeText(text); alert("All SKUs copied."); } catch {}
  };

  const copyOne = async (sku: string, qty = 1) => {
    try { await navigator.clipboard.writeText(`${sku} x${qty}`); } catch {}
  };
  ...
  return (
    <>
      <div className="flex flex-wrap gap-3">
        ...
        <Link href="/kits" className="rounded-full border border-zinc-700 px-4 py-2 hover:bg-zinc-900">
          View Kit Details
        </Link>
      </div>
      ...
      {showFallback && (
        <div className="fixed inset-0 z-50">
          <div className="absolute inset-0 bg-black/70 backdrop-blur-sm" onClick={() => setShowFallback(false)} />
          <div className="absolute inset-0 mx-auto flex max-w-lg items-center justify-center p-4">
            <div className="w-full rounded-2xl border border-zinc-800 bg-zinc-950 p-5">
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => setShowFallback(false)} className="rounded-md border border-zinc-700 px-2 py-1 text-xs hover:bg-zinc-900">
                  Close
                </button>
              </div>
              ...
              <div className="mt-3 rounded-xl border border-zinc-800 bg-zinc-900 p-3 text-sm">
                <div className="mb-2 text-xs uppercase tracking-wide text-zinc-400">{fallbackSkusTitle}</div>
                <ul className="grid gap-1">
                  {items.map((i) => (
                    <li key={i.sku} className="flex items-center justify-between gap-3 text-zinc-200">
                      <span>
              ...
              <div className="mt-4 flex gap-3">
                ...
                <button onClick={copySkus} className="rounded-full border border-zinc-700 px-4 py-2 hover:bg-zinc-900">
                  Copy All
                </button>
              </div>

              <p className="mt-3 text-xs text-zinc-500">
                Tip: Search by SKU, add to cart, repeat. If a SKU is out of stock, pick a close variant.
              </p>
            </div>
          </div>
        </div>
