...
type KitLite = { slug: string; title?: string; items?: Array<{ sku: string; qty?: number; title?: string }>; skus?: string[]; };
type SoloItem = { sku: string; title: string; url: string };

function titleCase(s: string) { return s ? s.replace(/^\w/, (c) => c.toUpperCase()) : s; }
function countsForKit(k: KitLite) {
  const arr = Array.isArray(k.items) ? k.items.map((i) => i.sku) : Array.isArray(k.skus) ? k.skus : [];
...
export default function ShopPage() {
  const kitsList = (kits as unknown as KitLite[]).map((k) => ({ slug: k.slug, title: k.title ?? `${titleCase(k.slug)} Kit`, stats: countsForKit(k) }));
    ...
    <section className="space-y-10 max-w-4xl">
      <header className="space-y-3">
        ...
        <div className="flex flex-wrap gap-3 pt-1">
          <a href={MYSHOP_BASE} target="_blank" rel="noopener noreferrer" className="btn">Open Storefront</a>
        </div>
      ...
      <div className="panel-elevated p-5 space-y-1">
        ...
        <p className="muted">The Safety Plan exists to save lives. We provide clean, effective wellness kits that meet real needs: hydration, energy, recovery, and rest. Profits support veteran suicide prevention and frontline support.</p>
      </div>
          ...
          {kitsList.map((k) => (
            <li key={k.slug} className="panel p-4 flex items-center justify-between">
              <div>
                ...
                <div className="muted text-sm">{k.stats.itemCount} items • {k.stats.skuCount} SKUs</div>
              </div>
              <div className="flex gap-2">
                <Link href={`/kits/${k.slug}`} className="btn-ghost">View items</Link>
                <a href={MYSHOP_BASE} target="_blank" rel="noopener noreferrer" className="btn">Buy now</a>
              </div>
            </li>
          ))}
        </ul>
      ...
      <section id="solo" className="space-y-4 scroll-mt-24">
        <div className="flex items-end justify-between">
          ...
          <a href={MYSHOP_BASE} target="_blank" rel="noopener noreferrer" className="btn-ghost text-sm">Open MyShop</a>
        </div>
        {solos.length === 0 ? (
          <div className="panel p-4"><p className="muted text-sm">Curated picks coming soon.</p></div>
        ) : (
          <ul className="grid gap-2">
            {solos.map((p) => (
              <li key={p.sku} className="glow-row">
                ...
                <div><a href={p.url} target="_blank" rel="noopener noreferrer" className="btn-ghost" aria-label={`Buy ${p.title}`}>Buy</a></div>
              </li>
            ))}
          </ul>
        )}
      ...
      <section id="tsp" className="space-y-4 scroll-mt-24">
        ...
        {!hasTsp ? (
          <div className="panel p-4"><p className="muted text-sm">Our custom gear is in progress. Check back soon.</p></div>
        ) : (
          <ul className="grid gap-2">
            {TSP_PRODUCTS.map((p) => {
              ...
              return (
                <li key={p.id} className="glow-row">
                  ...
                  <div className="flex gap-2">
                    {isInternal ? (
                      <Link href={href} className="btn-ghost">{label}</Link>
                    ) : (
                      <a href={href} target="_blank" rel="noopener noreferrer" className="btn-ghost">{label}</a>
                    )}
                  </div>
                </li>
              );
