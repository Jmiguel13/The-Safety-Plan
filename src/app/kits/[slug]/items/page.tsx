...
export default function KitItemsPage({ params }: { params: { slug: string } }) {
  ...
  const addAllUrl = items.length ? buildCartLink(items.map((i) => ({ sku: i.sku, qty: i.qty }))) : null;
    ...
    <section className="space-y-8 max-w-4xl">
      ...
      <header className="space-y-2">
        <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight">
          {title}<span aria-hidden="true">{" — "}</span>Items
        </h1>
        <p className="muted">Everything included in this kit. Buy individually or add the full kit.</p>
        <div className="flex flex-wrap gap-3 pt-2">
          {addAllUrl ? (
            <a href={addAllUrl} target="_blank" rel="noopener noreferrer" className="btn" aria-label="Add full kit to MyShop cart">
              Add full kit to cart
            </a>
          ) : null}

          {items.length > 0 ? <CopySkus items={items.map((i) => ({ sku: i.sku, qty: i.qty }))} /> : null}

          <a href={myShopLink("/")} target="_blank" rel="noopener noreferrer" className="btn-ghost">Open Storefront</a>
      ...
      {items.length === 0 ? (
        <div className="panel-elevated p-6">
          <p className="muted">No items yet. Add SKUs in <code>src/lib/kits.ts</code>.</p>
        </div>
      ) : (
            ...
            <li key={it.sku} className="glow-row">
              <div className="min-w-0">
                ...
                <div className="text-sm muted">
                  SKU: {it.sku}
                  {it.qty > 1 ? (<><span aria-hidden="true">{" • Qty "}</span>{it.qty}</>) : null}
                </div>
              ...
              <div className="flex items-center gap-2">
                {it.buy_url ? (
                  <a href={it.buy_url} target="_blank" rel="noopener noreferrer" className="link-chip">Buy</a>
                ) : (
                  <>
                    <a href={myShopLink(it.sku)} target="_blank" rel="noopener noreferrer" className="link-chip" aria-label={`View SKU ${it.sku} on MyShop`}>
                      View on MyShop
                    </a>
